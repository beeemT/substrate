...
// Services aggregates all dependencies the daemon owns and exposes to its
// callers. The TUI gets a view of this struct (re-typed as a views.Services
// alias) so it can reach the same services without owning their construction.
type Services struct {
	...
	Cfg    *config.Config
	// Adapters is the set of work-item adapters the daemon wired up.
	Adapters []adapter.WorkItemAdapter
	// RepoSources is the set of repository sources exposed by adapters.
	RepoSources []adapter.RepoSource
	// Harnesses holds the configured agent harnesses.
	Harnesses app.AgentHarnesses
	// Settings is the daemon-side settings service. Callers may swap in a
	// richer view-level implementation that satisfies SettingsService.
	Settings SettingsService
	// GitClient is the shared git work client.
	GitClient *gitwork.Client
	// Bus is the shared event bus.
	Bus *event.Bus
	// ReviewComments dispatches review comment fetches to source adapters.
...
// ForemanHarness returns the configured Foreman agent harness if one is
// available; otherwise it returns nil.
