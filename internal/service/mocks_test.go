...
// MockDocumentationRepository implements repository.DocumentationRepository for testing.
type MockDocumentationRepository struct {
	sources     map[string]domain.DocumentationSource
	byWorkspace map[string][]string
	err         error
}

func NewMockDocumentationRepository() *MockDocumentationRepository {
	return &MockDocumentationRepository{
		sources:     make(map[string]domain.DocumentationSource),
		byWorkspace: make(map[string][]string),
	}
}

func (m *MockDocumentationRepository) Get(ctx context.Context, id string) (domain.DocumentationSource, error) {
	if m.err != nil {
		return domain.DocumentationSource{}, m.err
	}
	ds, ok := m.sources[id]
	if !ok {
		return domain.DocumentationSource{}, repository.ErrNotFound
	}
	return ds, nil
}

func (m *MockDocumentationRepository) ListByWorkspaceID(ctx context.Context, workspaceID string) ([]domain.DocumentationSource, error) {
	if m.err != nil {
		return nil, m.err
	}
	var result []domain.DocumentationSource
	for _, id := range m.byWorkspace[workspaceID] {
		result = append(result, m.sources[id])
	}
	return result, nil
}

func (m *MockDocumentationRepository) Create(ctx context.Context, ds domain.DocumentationSource) error {
	if m.err != nil {
		return m.err
	}
	m.sources[ds.ID] = ds
	m.byWorkspace[ds.WorkspaceID] = append(m.byWorkspace[ds.WorkspaceID], ds.ID)
	return nil
}

func (m *MockDocumentationRepository) Update(ctx context.Context, ds domain.DocumentationSource) error {
	if m.err != nil {
		return m.err
	}
	if _, ok := m.sources[ds.ID]; !ok {
		return repository.ErrNotFound
	}
	m.sources[ds.ID] = ds
	return nil
...
func (m *MockDocumentationRepository) Delete(ctx context.Context, id string) error {
	if m.err != nil {
		return m.err
	}
	ds, ok := m.sources[id]
	...
	delete(m.sources, id)
	var newIDs []string
	for _, existingID := range m.byWorkspace[ds.WorkspaceID] {
		if existingID != id {
			newIDs = append(newIDs, existingID)
		}
	}
	m.byWorkspace[ds.WorkspaceID] = newIDs
	return nil
}
