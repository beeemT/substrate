...
func TestListSelectableInitiativesUsesExplicitGroup(t *testing.T) {
	a := makeAdapterWithConfig(t, config.GitlabConfig{Token: "token", BaseURL: "https://gitlab.example.com"}, roundTripFunc(func(req *http.Request) (*http.Response, error) {
	...
	if len(res.Items) != 1 || res.Items[0].ID != "12" {
		t.Fatalf("items = %+v, want one epic item", res.Items)
	}
}
...
func TestOnEventUpdatesStates(t *testing.T) {
	...
	if err := a.OnEvent(context.Background(), domain.SystemEvent{EventType: string(domain.EventPlanApproved), Payload: `{"external_id":"gl:issue:1234#42"}`}); err != nil {
...
func TestPlanApprovedAddsComments(t *testing.T) {
	...
	if err := a.OnEvent(context.Background(), domain.SystemEvent{EventType: string(domain.EventPlanApproved), Payload: `{"external_id":"gl:issue:1234#42","comment_body":"Overall plan text","external_ids":["gl:issue:1234#42","gl:issue:1234#43"]}`}); err != nil {
		t.Fatalf("plan approved: %v", err)
	}
...
func TestResolveProjectMilestoneUsesDirectEndpoint(t *testing.T) {
	...
	item, err := a.Resolve(context.Background(), adapter.Selection{Scope: domain.ScopeProjects, ItemIDs: []string{"77"}, Metadata: map[string]any{"project_id": int64(1234)}})
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	if item.ExternalID != "gl:milestone:1234" {
		t.Fatalf("ExternalID = %q, want gl:milestone:1234", item.ExternalID)
	}
	...
	if len(calls) < 1 || calls[0] != "GET /api/v4/projects/1234/milestones/77" {
...
func TestResolveInitiativeEpicUsesDirectEndpoint(t *testing.T) {
	...
	item, err := a.Resolve(context.Background(), adapter.Selection{Scope: domain.ScopeInitiatives, ItemIDs: []string{"12"}, Metadata: map[string]any{"group_id": int64(55)}})
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	if item.ExternalID != "gl:epic:12" {
		t.Fatalf("ExternalID = %q, want gl:epic:12", item.ExternalID)
	}
	...
	if len(calls) < 1 || calls[0] != "GET /api/v4/groups/55/epics/12" {
...
func TestResolveIssueTrackerRefs(t *testing.T) {
	a := makeAdapter(t, roundTripFunc(func(req *http.Request) (*http.Response, error) {
		switch req.URL.Path {
		case "/api/v4/projects/1234/issues/42":
			return jsonResponse(t, http.StatusOK, map[string]any{"iid": 42, "project_id": 1234, "title": "Issue 42", "description": "body", "labels": []any{}, "web_url": "https://gitlab.example.com/acme/rocket/-/issues/42"}), nil
		default:
			return jsonResponse(t, http.StatusOK, map[string]any{}), nil
		}
	}))
	item, err := a.Resolve(context.Background(), adapter.Selection{Scope: domain.ScopeIssues, ItemIDs: []string{"1234#42"}})
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
...
func TestResolveIssueTrackerRefsUsesProjectPath(t *testing.T) {
	a := makeAdapter(t, roundTripFunc(func(req *http.Request) (*http.Response, error) {
		switch req.URL.Path {
		case "/api/v4/projects/1234/issues/42":
			return jsonResponse(t, http.StatusOK, map[string]any{"iid": 42, "project_id": 1234, "title": "Issue 42", "description": "body", "labels": []any{}, "web_url": "https://gitlab.example.com/other-group/other-project/-/issues/42", "references": map[string]any{"full": "other-group/other-project#42"}}), nil
		default:
			return jsonResponse(t, http.StatusOK, map[string]any{}), nil
		}
	}))
